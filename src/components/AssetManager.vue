...
<template>
        ...
        <div class="vga-manager__body">
            ...
            <main class="vga-manager__content" ref="contentRef">
                ...
                <AssetGrid :items="store.fileList.value" :config="resolvedConfig" :copy-formats="copyFormats"
                    ...
                    @select="handleSelect" @confirm="(url, item) => emit('select', url, item)" @delete="handleDelete"
                    @retry="refresh" />
            </main>
        </div>
    ...
    <ConfirmDialog v-if="showConfirmDialog" :title="confirmConfig.title" :message="confirmConfig.message"
        :type="confirmConfig.type" @confirm="handleConfirm" @cancel="showConfirmDialog = false" />
</template>
