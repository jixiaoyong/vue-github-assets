...
import type { AssetItem, AssetManagerProps, AssetOperationResult, CopyFormat } from '@/types';
import {
    ChevronRight,
    FolderPlus,
    Home, Menu,
...
function openConfirm(config: { title: string; message: string; type?: 'info' | 'warning' | 'danger'; onConfirm: () => void }) {
    confirmConfig.value = {
        title: config.title,
...
// Watch path change to reset scroll
watch(() => store.currentPath.value, () => {
    nextTick(() => {
        if (contentRef.value) {
...
function handleConfirm(urls: string[], items: AssetItem[]) {
    emit('confirm', urls, items);
}

function handleCopy(content: string, format: CopyFormat, item: AssetItem) {
    emit('copy', content, format, item);
}

async function handleUpload(results: { success: boolean; url?: string; error?: Error }[]) {
    // Convert to AssetOperationResult format
    const successResults: AssetOperationResult[] = results
        .filter(r => r.success && r.url)
        .map(r => ({
            url: r.url!,
            item: { name: r.url!.split('/').pop() || '', path: '', size: 0, sha: '' } as AssetItem
        }));

    if (successResults.length > 0) {
        emit('upload', successResults);
    }

    // Emit errors for failed uploads
    results.filter(r => !r.success && r.error).forEach(r => {
        emit('error', r.error!);
    });

    // 🔧 FIX: AssetUploader 使用独立的 store 实例，所以需要刷新 AssetManager 的 store
    // AssetUploader uses its own store instance, so we need to refresh AssetManager's store
    await store.fetchList(store.currentPath.value);
}
...
async function handleDelete(item: AssetItem) {
    openConfirm({
        ...
        onConfirm: async () => {
            try {
                const url = store.getDisplayUrl(item);
                await store.remove(item);
                emit('delete', [{ url, item }]);
            } catch (e) {
                emit('error', e as Error);
            }
        }
...
<template>
    <div class="vga-manager vga-root">
        ...
        <div class="vga-manager__body">
            ...
            <!-- Main Content -->
            <main class="vga-manager__content" ref="contentRef">
                <!-- Uploader - shares store via uploadFn -->
                <AssetUploader v-if="showUploader" :config="resolvedConfig" :current-path="store.currentPath.value"
                    :disabled="store.isOperating.value" :upload-fn="(file, opts) => store.upload(file, opts)"
                    class="vga-manager__uploader" @upload="handleUpload" @error="handleError" />

                <!-- Grid with loading/error states -->
                <AssetGrid :items="store.fileList.value" :config="resolvedConfig" :copy-formats="copyFormats"
                    :loading="store.loading.value" :error="store.error.value" :is-operating="store.isOperating.value"
                    @confirm="handleConfirm" @copy="handleCopy" @delete="handleDelete" @retry="refresh" />
            </main>
        </div>
    ...
    <!-- Confirm Dialog -->
    <ConfirmDialog v-if="showConfirmDialog" :title="confirmConfig.title" :message="confirmConfig.message"
        :type="confirmConfig.type" @confirm="handleDialogConfirm" @cancel="showConfirmDialog = false" />
</template>
