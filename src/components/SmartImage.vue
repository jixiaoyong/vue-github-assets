...
// 加载超时时间 (ms) / Load timeout in milliseconds
...
function startLoadTimeout() {
  clearLoadTimeout();
  // 只在使用 CDN 时设置超时，raw URL 不设置
  if (currentCdn.value !== 'raw') {
    loadTimeoutId.value = setTimeout(() => {
      // 超时视为错误，尝试降级
      console.warn(`[SmartImage] Load timeout for ${currentCdn.value}, falling back...`);
      handleError();
    }, LOAD_TIMEOUT);
  }
}
...
// Watch for CDN change to restart timeout
watch(currentCdn, () => {
...
<template>
  <div class="vga-smart-image" :style="containerStyles" @click="imageError && retry()">
    <!-- Main image -->
    <img v-show="!imageError" :src="currentSrc" :alt="alt" :loading="lazy ? 'lazy' : 'eager'" :style="imageStyles"
      :class="[
    ...
    <div v-if="!imageLoaded && !imageError" class="vga-smart-image__loading">
    ...
    <div v-if="imageError" class="vga-smart-image__error">
